...
Navigation.registerComponent(
  "CP.CustomMainScreenTabButton",
  (): Object => CustomMainScreenTabButton
);
Navigation.registerComponent(
  "CP.CustomMainScreenTabMsgButton",
  (): Object => CustomMainScreenTabMsgButton
);
...
export class MainScreen extends React.Component<Props, States> {
  ...
  _onPressTutorial = () => {
    showModal({
      ...
      title: "教程",
      passProps: {
        images: [require("./img/tutorial1.png"), require("./img/tutorial2.png")]
      },
        ...
        onScroll={Animated.event(
          [{ nativeEvent: { contentOffset: { y: this.nScroll } } }],
          {
            ...
            listener: (event: Object) => {
              if (this.isReachEnd(event.nativeEvent)) this._onEndReached();
            }
          }
        )}
        style={styles.container}
      >
        <View onLayout={this.onHeaderLayout}>{this._renderHeader()}</View>
        {this._renderTabView()}
      </Animated.ScrollView>
    );
  }
